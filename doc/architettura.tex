...
Vorremmo far notare che il detector restituisce il risultato al solo nodo che invia la richiesta. Questo comportamento potrebbe sembrare limitante nel caso in cui il risultato della detection su di\todo{su di} un'immagine dovesse servire a più nodi, perché si potrebbe pensare di dover invocare più volte il servizio fornendo la stessa immagine in input. In realtà, questa cosa può essere evitata facendo in modo che un solo nodo invochi il servizio e pubblichi il risultato ottenuto su di\todo{su di} un apposito topic. Questo sistema, rispetto all'utilizzo del paradigma publish-subscribe, consente di conservare la possibilità di ricevere un feedback sul risultato dell'operazione. 
...
Il nodo di interfaccia verso Pepper è stato implementato per esporre agli altri nodi le funzionalità offerte dal NAOqi Framework\footnote{Il framework usato per programmare i robot dell'Aldebaran, risponde alle più comuni esigenze della robotica.}. , pertanto, circoscrivere l'utilizzo delle sue funzioni ad un singolo nodo permette agli altri nodi di utilizzare le versioni più recenti del linguaggio, evitando così di doversi appoggiare ad elementi di Python ormai deprecati.\todo{sistemare, specificando che non è stato fatto solo per python2} 

% Quest'ultimo è compatibile soltanto con Python 2, per cui bbiamo sfruttato l'interoperabilità tra nodi programmati in diversi linguaggi per confinare l'utilizzo di linguaggio, ormai deprecato, alla più piccola parte possibile dell'architettura. 
...
Quando il nodo server viene eseguito entra in \verb|spin|, in modo da restare in attesa di ricevere richieste. \todo{siccome non stoppiamo il servizio quando il nodo muore, verificare se restano i servizi appesi senza handler o vengono killati pure loro}
